...
function startUserDataUpdateCycle() {
    ...
    setInterval(() => {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            let userData = {};
            if (isUser(localStorage.key(i))) userData = JSON.parse(localStorage.getItem(localStorage.key(i)));
...
function isOldUser(user) {
    if (localStorage.getItem(user + "_theme") && localStorage.getItem(user + "_taskbarpos")) {
        return true;
    }
    return false;
}
