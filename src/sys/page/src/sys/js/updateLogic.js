...
class UpdateLogic {
    async checkForUpdates() {
        ...
        ConsoleNotifier.notifyStartService(
            "UpdateLogic.checkForUpdates: Checking for updates..."
        );
        ...
        if (latestVersionNumber > versionNumber) {
            ...
            ConsoleNotifier.notifyStartService(
                `UpdateLogic.checkForUpdates: Latest version already installed.`
            );
        }
    }
    ...
    async getVersionNumber() {
        return await fetch(
            "https://raw.githubusercontent.com/TWI-ArcOS/ArcOS-Environment/main/VERSION"
        )
    ...
    async downloadFile(file_url, targetPath) {
        ...
        let destinationDisplay = document.getElementById(
        ...
        if (progressBar && destinationDisplay && fileSizeDisplay) {
            ...
            req.on("end", function () {
                ...
                extract(
                    ...
                    () => {
                        if (uS)
                            uS.innerText =
                                "Extracted, moving to system directory...";

                        updateLogic.moveUpdateFiles();

                        if (uS)
                            uS.innerText =
                                "Updates downloaded! restarting in 10 seconds...";

                        notificationLogic.notificationService(
                            "ArcOS Updater",
                            "ArcOS has installed the updates, and it will restart in 10 seconds."
                        );
            ...
            errorLogic.sendError(
                "Unable to update",
                "The update application is not loaded. Please restart and try again."
            );
        }
    }
